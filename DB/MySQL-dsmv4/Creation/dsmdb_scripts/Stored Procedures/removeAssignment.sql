...
DELIMITER $$
USE `dsmv4`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `removeAssignment`(
IN subscriberId INT,
...
BEGIN
	...
	UPDATE Subscribers
		SET addressId = addressId, distributorId = NULL, remarks = remarks
    ...
    IF(personId IS NOT NULL) THEN
		IF(relationship = 'OWNER') THEN
			...
			INSERT INTO SubscribersRecord(addressId, distributorId, ownerId, remarks, userId)
				VALUES(addressId, NULL, personId, remarks, userId);
		ELSE
	...
	ELSE
			INSERT INTO SubscribersRecord(addressId, distributorId, remarks, userId)
				VALUES(addressId, NULL, remarks, userId);
    END IF;
END$$
