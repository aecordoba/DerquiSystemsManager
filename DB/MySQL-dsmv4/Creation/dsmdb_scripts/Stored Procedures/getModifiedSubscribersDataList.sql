USE `dsmv4`;
DROP procedure IF EXISTS `getModifiedSubscribersDataList`;

DELIMITER $$
USE `dsmv4`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `getModifiedSubscribersDataList`(
IN startDate DATE,
IN endDate DATE)
BEGIN
	...
	CREATE TEMPORARY TABLE LastBroadbandModificationOnPeriod ENGINE = MEMORY
		SELECT Subscribers.id AS subscriberId, Routers.name AS routerName, BroadbandRecord.username AS broadbandUsername
			FROM Subscribers
