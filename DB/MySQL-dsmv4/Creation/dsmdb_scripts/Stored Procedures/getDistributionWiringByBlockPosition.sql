USE `dsmv4`;
DROP procedure IF EXISTS `getDistributionWiringByBlockPosition`;

DELIMITER $$
USE `dsmv4`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `getDistributionWiringByBlockPosition`(
    ...
    Wiring.remarks AS wiringRemarks
		FROM Wiring
            ...
            LEFT JOIN OfficeCodes ON OfficeCodes.id = PhoneNumbers.officeCodeId
				WHERE BlockPositions.switchBlockId = switchBlockId AND BlockPositions.position = position;
END$$
