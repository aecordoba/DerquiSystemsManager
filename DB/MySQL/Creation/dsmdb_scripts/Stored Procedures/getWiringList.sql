USE `ac1`;
DROP procedure IF EXISTS `getWiringList`;

DELIMITER $$
USE `ac1`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `getWiringList`()
BEGIN
    ...
    Wiring.remarks AS wiringRemarks
		FROM Wiring
            ...
            LEFT JOIN OfficeCodes ON OfficeCodes.id = PhoneNumbers.officeCodeId
				ORDER BY OfficeCodes.areaId, OfficeCodes.code, PhoneNumbers.number;
